...
function displayScheduleInfo(schedule) {
    var scheduleDetailsContainer = document.getElementById("scheduleDetailsContainer");

    // 일정 정보를 동적으로 생성하여 컨테이너에 추가
    var scheduleDetailsHtml = `
        <div style="text-align:right; cursor:pointer; " class="close-button" onclick="closeScheduleDetails()">&times;</div> <!-- 닫기 버튼 추가 -->
        <h3 style="color: blue;">일정 정보</h3>
        <p class="input input-bordered w-full max-w-xs">순번: ${schedule.schedule_num}</p>
        <p class="input input-bordered w-full max-w-xs">일정번호: ${schedule.schedule_idx}</p>
        <p class="input input-bordered w-full max-w-xs datepicker">시작일: ${schedule.start_date}</p>
        <p class="input input-bordered w-full max-w-xs datepicker">종료일: ${schedule.end_date}</p>
        <p class="input input-bordered w-full max-w-xs">제목: ${schedule.schedule_subject}</p>
        <p class="input input-bordered w-full max-w-xs">내용: ${schedule.schedule_desc}</p>
        <a class="btn-text-link btn btn-outline btn-sm" style="margin: 10px;" onclick="if(confirm('정말 삭제하시겠습니까?') == false) return false;" href="/usr/calender/delete?idx=${schedule.schedule_idx}">삭제</a>
        <a class="btn-text-link btn btn-outline btn-sm" style="margin: 10px;" href="#" onclick="showeditForm(${schedule.schedule_idx})">수정</a>
    `;
...
function closeScheduleDetails() {
    var scheduleDetailsContainer = document.getElementById("scheduleDetailsContainer");
    scheduleDetailsContainer.innerHTML = ""; // 일정 정보 컨테이너를 비웁니다.
}
</script>
...
<div class="mt-40 mr-20" id="scheduleDetailsContainer"></div>

<div class="schedule_form mt-40">
		<div class="info"></div>
		<div style="text-align: right; cursor: pointer;" class="close-button" onclick="closeForm()">&times;</div>
		<!-- 닫기 버튼 추가 -->
...
<div class="schedule_editform mt-40 ml-20">
		<div class="info"></div>
		<div style="text-align: right; cursor: pointer;" class="close-button" onclick="closeEditForm()">&times;</div>
		<!-- 닫기 버튼 추가 -->
